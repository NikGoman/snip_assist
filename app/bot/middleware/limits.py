...
class LimitsMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        Если лимит не превышен, передаёт управление дальше.
        """
        ...
        if isinstance(event, Message) and event.text:
            ...
            # Если лимит не превышен, можно увеличить счётчик здесь,
            # чтобы избежать двойного вызова increment_usage в QueryService.
            # Однако, если QueryService будет вызываться не всегда,
            # это может привести к дисбалансу. Решение: оставить инкремент в QueryService,
            # или передавать флаг в data, чтобы QueryService знал, что инкремент уже был.
            # Для простоты и согласованности оставим инкремент в QueryService.
